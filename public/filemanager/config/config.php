...
chdir('../../');
    ...
    $countDirSize = new DirectorySizeCounting;

chdir($cwd);


if ($authService->getIdentity()->role != 3) {
    define('DIRECTORY_SIZE_LIMIT', 104857600);
    define('DIRECTORY_SIZE_WORD', 'ze'); 
}
else if ($authService->getIdentity()->role == 3) {
    define('DIRECTORY_SIZE_LIMIT', 1073741825); 
    define('DIRECTORY_SIZE_WORD', 'z');
}

// prepare path to login user directory
$dir_path = 'users/'.$authService->getIdentity()->id.'/source/';


$countDirSize->countSize(ROOT . '/source/' . $dir_path);
...
if ($authService->getIdentity()->role < 3) {
    ...
    if (DIRECTORY_SIZE_LIMIT <= $countDirSize->size + ($MaxSizeUpload * 1000000)) {
        $upload_files           = FALSE;
...
else if ($authService->getIdentity()->role > 2) {
    $upload_files	= TRUE;
    $duplicate_files 	= TRUE;
...
$config = array(
	...
	*/
	...
	|
	*/
	'thumbs_base_path' => '../thumbs/source/'.$dir_path.'thumbs/',
	...
	*/
	...
	'chmod_files'                             => $chmod_files, // change file permissions
	'chmod_dirs'                              => $chmod_dirs, // change folder permissions
	 ...
	 *******************/
/*
$aviary_key 	= "05c67d98d56c37b4";
$aviary_secret	= "33901e4cf6bc4fc9";
$aviary_version	= 3;
CLIENT SECRET 60dce806-aa42-44a0-aced-6dfdd4647f82
CLIENT ID (DEVELOPMENT MODE)* d92cdaa462c4453fa94b3844ac9ed5b9
 */
	'aviary_active'                           => false,
   'aviary_apiKey'                           => "60dce806-aa42-44a0-aced-6dfdd4647f82",
